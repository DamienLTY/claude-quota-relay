...
- **Statusline en argent restant** : `cqr credits budget 20 EUR` (ou `budget <compte> <montant>`) → la statusline affiche `cr 18,40 €` (somme de ce qui reste sur les comptes utilisables) au lieu d'un pourcentage. Le montant doit être saisi une fois : l'endpoint qui le contient (`/api/oauth/usage` → `extra_usage.monthly_limit`/`used_credits`/`currency`) **refuse les clés `claude setup-token`** (`403`, scope `user:profile` absent) ; seul le **pourcentage consommé** nous est accessible, et il suffit à convertir. Sans montant renseigné, on retombe sur le pourcentage.
